...
data VisitActionM m a =  SkipChildrenM 
                       | DoChildrenM
                       | ChangeToM a
                       | ChangeDoChildrenPostM (a, (a -> m a))


-------------------------------------------------------------------------------
doVisitM :: (Functor m, Monad m) => NanoVisitor a b -> VisitActionM m c -> (NanoVisitor a b -> c -> m c) -> c -> m c  
-------------------------------------------------------------------------------
...
{-
 - TODO: carry on with the monadic implementation
 -
-------------------------------------------------------------------------------
visitProgramM :: (Functor m, Monad m) =>  NanoVisitor a b -> Nano a b -> m (Nano a b)
-------------------------------------------------------------------------------
visitProgramM vis pgm = doVisitM vis (vProgram vis pgm) ch pgm
  where 
    ch vis p@(Nano {code = Src fs}) = 
      do  cd <- Src <$> visitStatementM vis <$> fs
          return $ p { code = cd }

 
-------------------------------------------------------------------------------
visitStatementM :: (Functor m, Monad m) =>  NanoVisitor a b -> Statement a -> m (Statement a)
-------------------------------------------------------------------------------
visitStatement vis st = doVisitM vis (vStatement vis st) ch st
  where
    ch vis s = 
      let ve  = visitExpression  vis
          vs  = visitStatement   vis
...
-------------------------------------------------------------------------------
visitExpressionM :: (Functor m, Monad m) =>  NanoVisitor a b -> Expression a -> Expression a
-------------------------------------------------------------------------------
visitExpression vis e = doVisit vis (vExpression vis e) ch e
  where 
    ch vis e = 
...
-------------------------------------------------------------------------------
visitCaseClauseM :: (Functor m, Monad m) =>  NanoVisitor a b -> CaseClause a -> CaseClause a
-------------------------------------------------------------------------------
visitCaseClause vis c = doVisit vis (vCaseClause vis c) ch c
  where 
    ch vis cc = 
...
-------------------------------------------------------------------------------
visitCatchClauseM :: (Functor m, Monad m) =>  NanoVisitor a b -> CatchClause a -> CatchClause a
-------------------------------------------------------------------------------
visitCatchClause vis c = doVisit vis (vCatchClause vis c) ch c
  where 
    ch vis c = 
...
-------------------------------------------------------------------------------
visitIdM :: (Functor m, Monad m) =>  NanoVisitor a b -> Id a -> Id a
-------------------------------------------------------------------------------
visitId vis i = doVisit vis (vId vis i) (flip const) i
            

-------------------------------------------------------------------------------
visitForInitM :: (Functor m, Monad m) =>  NanoVisitor a b -> ForInit a -> ForInit a
-------------------------------------------------------------------------------
visitForInit vis c = doVisit vis (vForInit vis c) ch c
  where 
    ch vis i = 
...
-------------------------------------------------------------------------------
visitJavaScriptM :: (Functor m, Monad m) =>  NanoVisitor a b -> JavaScript a  -> JavaScript a
-------------------------------------------------------------------------------
visitJavaScript vis js = doVisit vis (vJavaScript vis js) ch js
  where 
    ch vis (Script l ss) = Script l (visitStatement vis <$> ss) 
...
-------------------------------------------------------------------------------
visitInfixOpM :: (Functor m, Monad m) =>  NanoVisitor a b -> InfixOp -> InfixOp
-------------------------------------------------------------------------------
visitInfixOp vis io = doVisit vis (vInfixOp vis io) (flip const) io


-------------------------------------------------------------------------------
visitAssignOpM :: (Functor m, Monad m) =>  NanoVisitor a b -> AssignOp -> AssignOp
-------------------------------------------------------------------------------
visitAssignOp vis ao = doVisit vis (vAssignOp vis ao) (flip const) ao


-------------------------------------------------------------------------------
visitUnaryAssignOpM :: (Functor m, Monad m) =>  NanoVisitor a b -> UnaryAssignOp -> UnaryAssignOp 
-------------------------------------------------------------------------------
visitUnaryAssignOp vis uao = doVisit vis (vUnaryAssignOp vis uao) (flip const) uao

-------------------------------------------------------------------------------
visitPrefixOpM :: (Functor m, Monad m) =>  NanoVisitor a b -> PrefixOp -> PrefixOp
-------------------------------------------------------------------------------
visitPrefixOp vis po = doVisit vis (vPrefixOp vis po) (flip const) po


-------------------------------------------------------------------------------
visitPropM :: (Functor m, Monad m) =>  NanoVisitor a b -> Prop a -> Prop a
-------------------------------------------------------------------------------
visitProp vis p = doVisit vis (vProp vis p) ch p
  where 
    ch vis p = 
...
-------------------------------------------------------------------------------
visitLValueM :: (Functor m, Monad m) =>  NanoVisitor a b -> LValue a -> LValue a
-------------------------------------------------------------------------------
visitLValue vis lv = doVisit vis (vLValue vis lv) ch lv
  where 
    ch vis lv = 
...
-------------------------------------------------------------------------------
visitVarDeclM :: (Functor m, Monad m) =>  NanoVisitor a b -> VarDecl a -> VarDecl a
-------------------------------------------------------------------------------
visitVarDecl vis vd = doVisit vis (vVarDecl vis vd) ch vd
  where 
    ch vis (VarDecl l i me) = VarDecl l (visitId vis i) (visitExpression vis <$> me)
...
-------------------------------------------------------------------------------
visitForInInitM :: (Functor m, Monad m) =>  NanoVisitor a b -> ForInInit a   -> ForInInit a   
-------------------------------------------------------------------------------
visitForInInit vis fi = doVisit vis (vForInInit vis fi) ch fi
  where 
    ch vis (ForInVar i)   = ForInVar  $ visitId vis i
