...
--------------------------------------------------------------------------------
verifyFile cfg f fs = either (A.NoAnn,) id <$> runEitherIO $
  do  p   <- EitherIO   $ parseNanoFromFiles fs
...
--------------------------------------------------------------------------------
consNano :: RefScript -> ClassHierarchy F.Reft -> CGM ()
--------------------------------------------------------------------------------
consNano p@(Rsc {code = Src fs}) cha
...
initGlobalEnv pgm@(Rsc { code = Src ss }) cha = freshenCGEnvM g
  where
    g     = CGE nms bnds ctx pth cha fenv grd cst mut tThis
...
initCallableEnv l g f fty s
  = do  g1 <- cgEnvAdds ("init-func-" ++ ppshow f ++ "-0") params g0
        ...
        freshenCGEnvM g2
  where
    g0     = CGE nms bnds ctx pth cha fenv grd cst mut tThis
    ...
    params = [(x, VI WriteLocal Initialized t) | B x t <- xts ]
...
consFun g (FunctionStmt l f xs body)
  = case envFindTy f (cge_names g) of
      Just (VI _ _ t) -> cgFunTys l f xs t >>= mapM_ (consCallable l g f xs body)
  ...
  where
    ...
    xts | Just (TObj ms _) <- expandType Coercive (envCHA g0) tThis
        = sortBy c_sym [ B x t' | (x, FI _ _ t) <- F.toListSEnv (tm_prop ms)
                                , let t' = unqualifyThis g0 tThis t ]
        | otherwise
        = []

    out | Just (TObj ms _) <- expandType Coercive (envCHA g0) tThis
        = [ f | (f, FI _ m _) <- F.toListSEnv (tm_prop ms), isImm m ]
        | otherwise
        = []
